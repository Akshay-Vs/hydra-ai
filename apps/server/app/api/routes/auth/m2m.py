...
from app.models.enums import GrantType
...
async def get_access_token(
    token_request: TokenRequest, db: Session = Depends(get_db_session)
):
    """
    ...
    """
    # Validate grant type
    if token_request.grant_type != GrantType.CLIENT_CREDENTIALS:
        raise HTTPException(
    ...
    logger.info(
        f"Authenticating organization {token_request.organization_id} with credential id {token_request.credential_id}"
    )
    if not verify_organization_credentials(
        token_request.organization_id,
        token_request.credential_id,
    ...
    token_data = {
        "sub": token_request.organization_id,  # Organization ID as subject
...
async def verify_token(
    current_client: SessionClient = Depends(get_current_client),
):
    """
...
async def list_active_sessions(
    ...
    for auth_session in active_sessions:
        if session_store.is_session_valid(auth_session.token_jti):
            sessions_response.append(
                {
                    "session_id": auth_session.id,
                    "token_jti_hash": auth_session.token_jti[:16]
    ...
    return {
        ...
        "active_sessions": sessions_response,
    }
