...
def create_ws_auth_decorators(
    session_manager: SessionManager, sio: socketio.AsyncServer
):
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def wrapper(sid: str, *args, **kwargs) -> Any:
                ...
                if not PermissionChecker.has_permission(session, permission):
                    logger.warning(
                        f"Permission denied for sid: {sid}, permission: {permission}"
                    )
                ...
                logger.debug(
                    f"Executing {func.__name__} for sid: {sid} with session: {session}"
                )
